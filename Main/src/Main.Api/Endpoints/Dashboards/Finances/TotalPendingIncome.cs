...
public class TotalPendingIncome(AppDbContext dbContext) : Endpoint<TotalIncomeRequest, TotalIncomeResponse> {
    public override void Configure() {
        Get("dashboard/finances/total-pending-income"); // Distinct route
        Version(1);
    ...
    public override async Task HandleAsync(TotalIncomeRequest req, CancellationToken ct) {
        ...
        var query = dbContext.Incomes
            .Where(i => i.CurrencyId == req.CurrencyId && i.IncomeStatus == IncomeStatus.Pending);
